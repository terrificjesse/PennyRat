...
export function BudgetMeter({
  ...
  const bucketRows = BUCKET_KEYS.map((bucket) => {
    const status = budget.perBucket[bucket];
    const bucketValueText = `${formatCents(status.spent)} selected of ${formatCents(
      status.planned,
    )}.${status.over ? " Over this bucket budget." : ""}`;
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p
            aria-live="polite"
            aria-atomic="true"
            className={`${compact ? "text-2xl" : "text-3xl"} font-bold tracking-[-0.04em] ${
      ...
      <p className="mt-2 text-xs leading-5 text-muted-foreground">
        {budget.overTotal
          ? "Selections exceed the total trip budget. Remove or replace a choice to continue."
          : `Total plan: ${formatCents(total)}`}
      </p>
      ...
      {compact ? (
        <details className="mt-4 border-t border-border pt-4">
          ...
          <div className="mt-4 space-y-4">{bucketRows}</div>
        </details>
      ) : (
        <div className="mt-7 space-y-4 border-t border-border pt-6">{bucketRows}</div>
      )}
    </Card>
  );
}
